...
class Application(tk.Frame):
    ...
    def initNewGame(self):
        ...
        self.Replacement = [i for i in range(self.NumOfRows*self.NumOfColumns-1)]
    ...
    def is_sorted(self, replacement):
    ...
    def movePuzzle(self, puzzle):
        ...
        if (abs(row - self.FreePuzzleRow) == 1 and column == self.FreePuzzleColumn) or (abs(column - self.FreePuzzleColumn) and row == self.FreePuzzleRow):
            ...
            self.showWidgets()
            if (self.is_sorted(self.Replacement)):
